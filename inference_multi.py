...
def inference_multi_imagefile(_net, _image_path, _print=False, _show=False, _thresh=0.3):
    flag, ims = cv.imreadmulti(_image_path, flags=cv.IMREAD_ANYDEPTH)
    cnt = 0
    rst = {}
    for orig_image in ims:
        orig_image = (orig_image >> 4).astype(np.uint8)
        orig_image = cv.cvtColor(orig_image, cv.COLOR_GRAY2RGB)
        ...
        input_image = cv.blur(input_image, ksize=(5, 5))
...
if __name__ == '__main__':
    ...
    multi_image_path = '/home/raosj/PycharmProjects/spectral_face/srcs/image_data/0523/0523_fake.tif'
    preds = inference_multi_imagefile(net, multi_image_path, _print=True, _show=True)
    savemat('/home/raosj/PycharmProjects/spectral_face/srcs/image_data/0523/0523_fake_boxes.mat', preds)
