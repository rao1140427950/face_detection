...
Original code: 2018 Pierluigi Ferrari
"""
...
class SSDLoss(tf.keras.losses.Loss):
    ...
    def __init__(self, neg_pos_ratio=3, n_neg_min=0, alpha=1.0, **kwargs):
        """
        Arguments:
    ...
    def log_loss(y_true, y_pred):
        ...
        log_loss = -tf.reduce_sum(y_true * K.log(y_pred), axis=-1)
        return log_loss
    ...
    def call(self, y_true, y_pred):
        ...
        def f2():
            ...
            negatives_keep = K.cast_to_floatx(tf.reshape(negatives_keep, [batch_size, n_boxes])) # Tensor of shape (batch_size, n_boxes)
            # ...and use it to keep only those boxes and mask all other classification losses
            neg_class_loss0 = tf.reduce_sum(classification_loss * negatives_keep, axis=-1) # Tensor of shape (batch_size,)
            return neg_class_loss0
        ...
        total_loss = total_loss * K.cast_to_floatx(batch_size)
