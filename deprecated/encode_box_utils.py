...
Original code: https://github.com/rao1140427950/ssd_keras/blob/master/ssd_encoder_decoder/ssd_input_encoder.py
"""
...
class SSDInputEncoder:
    ...
    def __init__(self,
        ...
        if scales is not None:
            if len(scales) != predictor_sizes.shape[0] + 1: # Must be two nested `if` statements since `list` and `bool` cannot be combined by `&`
                raise ValueError("It must be either scales is None or len(scales) == len(predictor_sizes)+1, \
                but len(scales) == {} and len(predictor_sizes)+1 == {}".format(len(scales), len(predictor_sizes)+1))
            scales = np.array(scales)
            if np.any(scales <= 0):
                raise ValueError("All values in `scales` must be greater than 0, but the passed list of scales is\
                 {}".format(scales))
        else: # If no list of scales was passed, we need to make sure that `min_scale` and `max_scale` are valid values.
            if not ((0 < min_scale) and (min_scale <= max_scale)):
                raise ValueError("It must be 0 < min_scale <= max_scale, but it is min_scale = {} and max_scale =\
        ...
        if not (aspect_ratios_per_layer is None):
            if len(aspect_ratios_per_layer) != predictor_sizes.shape[0]: # Must be two nested `if` statements since `list` and `bool` cannot be combined by `&`
                raise ValueError("It must be either aspect_ratios_per_layer is None or len(aspect_ratios_per_layer)\
                 == len(predictor_sizes), but len(aspect_ratios_per_layer) == {} and len(predictor_sizes)\
                  == {}".format(len(aspect_ratios_per_layer), len(predictor_sizes)))
            for aspect_ratios in aspect_ratios_per_layer:
                if np.any(np.array(aspect_ratios) <= 0):
                    raise ValueError("All aspect ratios must be greater than zero.")
        else:
            if aspect_ratios_global is None:
                raise ValueError("At least one of `aspect_ratios_global` and `aspect_ratios_per_layer`\
                 must not be `None`.")
    ...
    def __call__(self, ground_truth_labels, diagnostics=False):
        ...
        Arguments:
            ground_truth_labels (list or np.array): A python list of length `batch_size` that contains one 2D Numpy array
                for each batch image, or 3D Numpy array. Each such array has `k` rows for the `k` ground truth bounding
                 boxes belonging to the respective image, and the data for each ground truth bounding box has the format
                `(class_id, xmin, ymin, xmax, ymax)`, and `class_id` must be
        ...
        for i in range(batch_size): # For each batch item...
            ...
            if np.any(labels[:,[xmax]] - labels[:,[xmin]] <= 0) or np.any(labels[:,[ymax]] - labels[:,[ymin]] <= 0):
                raise DegenerateBoxError("SSDInputEncoder detected degenerate ground truth bounding boxes for batch\
                 item {} with bounding boxes {}, ".format(i, labels) + "i.e. bounding boxes where xmax <= xmin\
                  and/or ymax <= ymin. Degenerate ground truth " + "bounding boxes will lead to NaN errors during\
                   the training.")
    ...
    def generate_anchor_boxes_for_layer(self,
        ...
        Arguments:
            ...
            diagnostics (bool, optional): If true, the following additional outputs will be returned:
                ...
                to be detected.
            this_steps: see above
            this_offsets: see above
        ...
        if this_steps is None:
            ...
            if isinstance(this_steps, (list, tuple)) and (len(this_steps) == 2):
                ...
                raise ValueError("Unsupported value for `this_steps`: {}.".format(this_steps))
        ...
        if this_offsets is None:
            ...
            if isinstance(this_offsets, (list, tuple)) and (len(this_offsets) == 2):
                ...
                raise ValueError("Unsupported value for `this_offsets`: {}.".format(this_offsets))
